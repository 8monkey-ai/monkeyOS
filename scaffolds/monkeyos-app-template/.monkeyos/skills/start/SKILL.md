...
4. Start local Supabase, reset migrations and deterministic seed, store its public local client configuration through `bun run config:local`, and generate types. A new scaffold seeds Auth and membership only; seed synthetic business records only for real routed modules.
