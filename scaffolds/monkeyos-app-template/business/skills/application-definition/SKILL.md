...
The generic scaffold intentionally assumes no business domain and owns no placeholder business records. It provides authentication, app-local membership, access administration, configuration, testing, and delivery only. No business workflow, state machine, calculation, approval, retention rule, integration, KPI, or exception path is authoritative until an owner confirms it.
...
- Capture actors, permissions, states, transitions, validations, approvals, exceptions, ownership, traceability, retention, integrations, and acceptance scenarios in that skill.
- Add only real application-owned schema, deterministic synthetic records, RLS policies, and tests derived from the confirmed process.
...
4. New business tables include explicit grants, RLS, deterministic seeds, generated types, and negative authorization tests.
