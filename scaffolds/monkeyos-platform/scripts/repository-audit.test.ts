...
test("detects versioned and unreferenced business skills", async () => {
  ...
  await writeFile(
    ...
    '{"version":"1.0.0","scripts":{"test:container":"bun scripts/test-container.ts"}}',
  );
  await writeFile(join(root, "tsconfig.json"), '{"compilerOptions":{"allowJs":false}}');
  await mkdir(join(root, "scripts"));
  await writeFile(join(root, "scripts/test-container.ts"), "export {};\n");
  ...
  expect(findings.map((finding) => finding.message).join("\n")).toContain(
    "application-owned test orchestration",
  );
  expect(findings.map((finding) => finding.message).join("\n")).toContain(
