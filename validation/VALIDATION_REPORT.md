...
- `scaffolds/monkeyos-app-template` — generic application scaffold, version 2.2.0
- `scaffolds/monkeyos-platform` — generic organization-level platform repository, version 2.2.0

## Results

| Area | Result |
| --- | --- |
| Platform formatting, lint, typecheck and unit tests | PASS — 17 tests, including direct Node-adapter rejection |
...
| React Router Framework Mode on Bun | PASS — Bun-forced dev/build/typegen, standard route config and root document, generated client and Web Streams SSR server builds, and thin native request adapter |
...
- The application uses React Router's standard Framework Mode conventions: `react-router.config.ts`, `src/root.tsx`, `src/routes.ts`, route modules, generated type data, and generated client/Web Streams server builds. The CLI is explicitly forced onto Bun. A small `server.ts` serves generated assets and delegates application requests to React Router's standard `createRequestHandler`; it owns no application middleware or framework behavior.
- Bun is not pinned to a minor line. It is the sole JavaScript runtime, package manager, application server, and test runner; GitHub workflows install latest stable Bun, and the Dockerfile uses only the moving `oven/bun:alpine` base. There is no Node executable, direct Node adapter package, Node type entry, or Node container base. Committed `bun.lock` preserves exact dependency resolutions for each tested SHA.
